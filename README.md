...
auto-refreshes when files change on your hard drive.
    ...
    dotnet run --verbosity normal --project ./src/World/World.csproj
    npm --prefix ./src/World/ClientApp start

npm is also used to manage CSS and JavaScript dependencies used in this application. You can upgrade dependencies by
specifying a newer version in [package.json](package.json). You can also run `npm update` and `npm install` to manage dependencies.
Add the `help` flag on any command to see how you can use it. For example, `npm help update`.

The `npm --prefix ./src/World/ClientApp run` command will list all of the scripts available to run for this project.

### Managing dependencies

For example, to add [Leaflet][] library as a runtime dependency of your application, you would run following command:

    npm --prefix ./src/World/ClientApp install --save --save-exact leaflet

To benefit from TypeScript type definitions from [DefinitelyTyped][] repository in development, you would run following command:

    npm --prefix ./src/World/ClientApp install --save-dev --save-exact @types/leaflet

Then you would import the JS and CSS files specified in library's installation instructions so that [Webpack][] knows about them:
Edit [World/ClientApp/app/vendor.ts](World/ClientApp/app/vendor.ts) file:

```
import 'leaflet/dist/leaflet.js';
```

Edit [World/ClientApp/content/css/vendor.css](World/ClientApp/content/css/vendor.css) file:
...
will generate few files:
    ...
    create World/ClientApp/src/app/my-component/my-component.component.html
...
To build the artifacts and optimize the world application for production, run:
    ...
    cd ./src/World
    rm -rf ./src/World/wwwroot
    dotnet publish --verbosity normal -c Release -o ./app/out ./World.csproj

The `./src/World/app/out` directory will contain your application dll and its depedencies.
...
3. Run `` dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"World" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml" ``
...
```bash
docker build -f ./Dockerfile-Back -t world .
```

Then run:

```bash
docker run -p 8080:80 world
```
